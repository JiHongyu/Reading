...
Echo，Daytimeme，Domain，TFTP，SNMP
...
三次挥手方法意味着连接的双方几乎同时关闭，这是其最大的缺点，因为这需要双方同步约定关闭的时间，这是需要开销的。然而四次挥手可以使一端连接关闭而另一端保持打开，即保证了单向的数据传输。注意，关闭是关闭的 **数据** 发送行为，控制消息报文还是可以发送的。
